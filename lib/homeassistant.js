...
exports.homedDiscovery = function (data, device, peripheral, serviceId) {
  ...
  for (let dataKey in data) {
    if (deviceTypes[dataKey] !== undefined && !devicesExposes[deviceKey].includes(dataKey)) {
      ...
      //  }
      // options = options + "\"" + dataKey + "\":{\"type\": \"sensor\", \"class\": \"" + dataKey + "\", \"state\": \"measurement\", \"unit\": \"" + deviceTypes[dataKey] + "\", \"round\": 1},";
    }
  }
  exposes = exposes + "]";
  ...
  let payload = {
    ...
    "data": {
      ...
      "note": "",
      "real": true,
      "exposes": devicesExposes[deviceKey],
      "options": { options },
    }
  };
  mqtt.send(configTopic, JSON.stringify(payload), { retain: true });
