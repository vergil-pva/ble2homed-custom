...
exports.homedDiscovery = function (data, device, peripheral, serviceId) {
  ...
  for (let dataKey in data) {
    if (deviceTypes[dataKey] !== undefined && !devicesExposes[deviceKey].includes(dataKey)) {
      ...
      // Ensure payload.data.options exists and is initialized properly
      if (!payload.data.options[dataKey]) {
        payload.data.options[dataKey] = [];
      }

      // Assign devicesOptions[deviceKey] to payload.data.options[dataKey]
      payload.data.options[dataKey] = devicesOptions[deviceKey];
      console.log(dataKey);
      console.log(payload.data.options[dataKey]);
      console.log(devicesOptions[deviceKey]);
