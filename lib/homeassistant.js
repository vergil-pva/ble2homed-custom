...
exports.homedDiscovery = function (data, device, peripheral, serviceId) {
  ...
  let configTopic = `homed/command/custom`
  let payload = {
    "action": "updateDevice",
    "device": [id],
    "data": {
      ...
      "cloud": false,
      "discovery": false,
      "id": (device.name_is_mac ? uuid : device.name),
      "name": (device.name_is_mac ? uuid : device.name),
      "note": "",
      "real": true,
    }
  };

  mqtt.send("Data", JSON.stringify(data), { retain: false });
  mqtt.send("device", JSON.stringify(device), { retain: false });
  //mqtt.send("peripheral", JSON.stringify(peripheral), { retain: false });
  //mqtt.send("serviceId", JSON.stringify(serviceId), { retain: false });
  ...
  // Функция для проверки, существует ли уже dataKey в exposes
  //function isDataKeyExposed(dataKey) {
  //  return exposes.some(exposesInfo => exposesInfo.dataKey === dataKey);
  // }

  for (let dataKey in data) {
    if (deviceTypes[dataKey] !== undefined && !devicesExposes[deviceKey].includes(dataKey)) {

      devicesExposes[deviceKey].push(dataKey);
      payload.exposes = devicesExposes[deviceKey];


      let options = {};
      options[dataKey]["class"] = dataKey;
      options[dataKey]["round"] = 1;
      options[dataKey]["state"] = "measurement";
      options[dataKey]["type"] = "sensor";
      options[dataKey]["unit"] = deviceTypes[dataKey]["unit_of_measurement"];
      devicesOptions[deviceKey].push(options);
      //  if (binaryDevices.includes(dataKey)) {
      //    exposes = exposes + "switch_" + dataKey + ", ";
      //  } else {
      //    exposes = exposes + dataKey + ", ";
      //  }
