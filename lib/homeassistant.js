...
exports.homedDiscovery = function (data, device, peripheral, serviceId) {
  ...
  for (let dataKey in data) {
    if (deviceTypes[dataKey] !== undefined && !discoverySend[id + serviceId + dataKey]) {
      ...
      options = options + "\"" + dataKey + "\":{\"type\": \"sensor\", \"class\": \"" + dataKey + "\", \"state\": \"measurement\", \"unit\": \"°C\", \"round\": 1},";


    }
  }
  exposes = exposes + "]";
  let configTopic = `homd/command/custom/`
  let payload = {
    "action": "updateDevice",
    "device": [id],
    "data": {
      ...
      "cloud": false,
      "discovery": false,
      "id": (device.name_is_mac ? uuid : device.name) + " " + joinKey([serviceId, dataKey]),
