...
async function onDiscovery(peripheral) {
  ...
  if (entered) {
    ...
    mqtt.send(dev.presence_topic, "1", { retain: false });
  }
  var mqttData = {
  ...
  if (peripheral.advertisement.serviceData) {
    peripheral.advertisement.serviceData.forEach(function (d) {
      ...
      if (decoded !== d.data) {
        ...
        if (config.homeassistant) homeassistant.homedDiscovery(decoded, dev, peripheral, d.uuid);
...
function checkForPresence() {
  ...
  Object.keys(inRange).forEach(function (addr) {
    ...
    if (inRange[addr].lastSeen < timeout) {
      mqtt.send(inRange[addr].dev.presence_topic, "0", { retain: false });
      delete inRange[addr];
    }
  });
}
...
exports.sendMQTTPresence = function () {
  ...
  for (let addr in inRange) {
    mqtt.send(inRange[addr].dev.presence_topic, "1", { retain: false });
  }
  for (let mac in devices.list) {
    if (devices.list[mac].known)
      mqtt.send(devices.list[mac].presence_topic, (devices.list[mac].mac in inRange) ? "1" : "0", { retain: false });
  }
}
