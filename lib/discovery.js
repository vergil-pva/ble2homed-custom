...
async function onDiscovery(peripheral) {
  ...
  if (entered) {
    ...
    if (config.retain) { mqtt.send(dev.presence_topic, '{"status":"online"}', { retain: true }); } else {
...
function checkForPresence() {
  ...
  Object.keys(inRange).forEach(function (addr) {
    ...
    if (inRange[addr].lastSeen < timeout) {
      if (config.retain) { mqtt.send(inRange[addr].dev.presence_topic, '{"status":"offline"}', { retain: true }); } else { mqtt.send(inRange[addr].dev.presence_topic, '{"status":"offline"}', { retain: false }); }
      delete inRange[addr];
    }
  });
}
...
exports.sendMQTTPresence = function () {
  ...
  for (let addr in inRange) {
    if (config.retain) { mqtt.send(inRange[addr].dev.presence_topic, '{"status":"online"}', { retain: true }); } else { mqtt.send(inRange[addr].dev.presence_topic, '{"status":"online"}', { retain: false }); }
  }
  for (let mac in devices.list) {
    if (devices.list[mac].known) {
      if (config.retain) { mqtt.send(devices.list[mac].presence_topic, (devices.list[mac].mac in inRange) ? '{"status":"online"}' : '{"status":"offline"}', { retain: true }); } else { mqtt.send(devices.list[mac].presence_topic, (devices.list[mac].mac in inRange) ? '{"status":"online"}' : '{"status":"offline"}', { retain: false }); }
    }
  }
}
