...
async function onDiscovery(peripheral) {
  ...
  if (entered) {
    ...
    mqtt.send(dev.presence_topic, "1", { false });
  }
  var mqttData = {
...
function checkForPresence() {
  ...
  Object.keys(inRange).forEach(function (addr) {
    ...
    if (inRange[addr].lastSeen < timeout) {
      mqtt.send(inRange[addr].dev.presence_topic, "0", { false });
      delete inRange[addr];
    }
  });
}
...
exports.sendMQTTPresence = function () {
  ...
  for (let addr in inRange) {
    mqtt.send(inRange[addr].dev.presence_topic, "1", { false });
  }
  for (let mac in devices.list) {
    if (devices.list[mac].known)
      mqtt.send(devices.list[mac].presence_topic, (devices.list[mac].mac in inRange) ? "1" : "0", { false });
  }
}
