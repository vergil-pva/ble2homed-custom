...
function createDevice(mac, name = "", settings) {
  let name_is_mac = false;
  mac = mac.toLowerCase();
  if (name === "") {
    name = mac;
    name_is_mac = true;
  }
  let device = { mac, name, name_is_mac, ...defaultSettings, ...settings };

  device.json_state_topic = config.mqtt_prefix + "/fd/custom/" + device.name;
  device.presence_topic = config.mqtt_prefix + "/device/custom/" + device.name;
