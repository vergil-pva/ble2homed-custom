...
class ParserAtc {
  ...
  parse() {
    if (this.buffer.length === 15) { // pvvx
      ...
      return {
        temperature: this.buffer.readInt16BE(6) / 10,
        humidity: this.buffer.readUInt8(8),
        battery: this.buffer.readUInt8(9),
        voltage: this.buffer.readInt16BE(10) / 1000,
        type: "ATC (ATC1441)"
      }
    } else if (this.buffer.length === 11) {
      const decoded         = this.decryptPayload();
      const voltage = 2.2 + (3.1 - 2.2) * (decoded.readInt8(3) / 100);
      return {
        temperature: decoded.readInt16LE(0) / 100,
      ...
      const voltage = 2.2 + (3.1 - 2.2) * (battery / 100);
      const trigger         = decoded.readInt8(2) >> 7;

      return {
        temperature: decoded.readUInt8(0) / 2 - 40,
